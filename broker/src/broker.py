...
sys.path.insert(0, str(OUTER_ADMIN))
sys.path.insert(0, str(OUTER_ADMIN / "admin_modules"))
...
admin_pkg.__path__.append(str(OUTER_ADMIN))
...
def save_report(payload: dict) -> str:
    ...
    LOG.info("Report %s saved (%s defects)", rpt.id,
             len(payload.get("defects", [])))
    return str(rpt.id)
